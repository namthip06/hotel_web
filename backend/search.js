...
function search_hotel(data) {
    ...
    data.then(function(data) {
        Object.keys(data).forEach(function(key) {
            count++
            text += `
                <div class="row gx-lg-5 py-5 border-bottom border-secondary-subtle">
                    ...
                    <div class="col-auto align-self-end text-end">
                        ...
                        <button type="button" class="btn btn-dark fw-bold book_btn" onclick="submitHotelId(${data[key]['Id']})">Book Now</button>
                    </div>
                </div>
            `
        });
...
function submitHotelId(hotel_id) {
    localStorage.setItem('lode_hotel_id', JSON.stringify(hotel_id));
